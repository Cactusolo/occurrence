...
class HiveDataTypes {
  ...
  public static String typeForTerm(Term term, boolean verbatimContext) {
    ...
    if (GbifTerm.gbifID == term) {
      return TYPED_TERMS.get(term);

    } else if (verbatimContext) {
      return TYPE_STRING; // verbatim are always string

    } else if (TYPED_TERMS.containsKey(term)) {
      return TYPED_TERMS.get(term); // interpreted term with a registered type

    } else {
      return TYPE_STRING; // default type for non registered terms
